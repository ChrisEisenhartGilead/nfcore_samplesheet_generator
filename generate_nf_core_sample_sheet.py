...
def validate_options(options):
    """
    Use this to enable conditional mandatory arguments (ex Sarek)
    """
    if options.pipeline == "sarek":
        if options.sarek_status is None:
            print("The --sarek_type option is required for the 'sarek' pipeline.")
            exit(1)
...
def generate_nf_core_sarek_sample_sheet(paired_files, in_status, mapping_file):
    """
    ...
    for data in paired_files:
        ...
        sample_sheet_entry = (
            f"{sample_name},NA,{status},{sample_name},{lane},{r1_fastq},{r2_fastq}"
        )
        results.append(sample_sheet_entry)
    return results
...
def main(args):
    ...
    if options.pipeline == "rnaseq":
        ...
        output_data = generate_nf_core_sarek_sample_sheet(
            paired_files, options.sarek_status, options.sarek_mapping_file
        )
